...
export interface QuizQuestion {
  id: string;
  type: 'True/False' | 'Single Select' | 'Yes/No' | 'Multi-Select';
  question: string;
  options?: string[];
...
// Simulated quiz generation - in a real app, this would call an AI API
export const generateQuizQuestions = async (config: QuizConfig): Promise<QuizQuestion[]> => {
  // Simulate API delay
  ...
  for (let i = 0; i < config.cardCount; i++) {
    const randomType = availableTypes[Math.floor(Math.random() * availableTypes.length)] as QuizQuestion['type'];
    ...
    switch (randomType) {
      ...
      case 'Multi-Select':
        ...
        const correctCount = Math.floor(Math.random() * 3) + 2; // 2-4 correct answers
